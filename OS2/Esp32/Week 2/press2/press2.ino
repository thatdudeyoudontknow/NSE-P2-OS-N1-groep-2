...
void addAuditEntryAndPrint() {
  ...
  if (num_audit_entries < max_audit_entries) {
    // Voeg de timestamp toe aan het einde van de audit_queue
    xQueueSendToBack(audit_queue, glob_time_buf, 0);
    num_audit_entries++;
  } else {
    // Verwijder de oudste timestamp uit de audit_queue
    char old_entry[glob_buf_size];
    xQueueReceive(audit_queue, &old_entry, 0);

    // Voeg de nieuwe timestamp toe aan het einde van de audit_queue
    xQueueSendToBack(audit_queue, glob_time_buf, 0);
  }
